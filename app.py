...
def login_screen():
    ...
    # Nota UX/seguridad:
    # - Siempre mostramos 3 vías de ingreso (contraseña / código 1-uso / bootstrap).
    # - El bootstrap requiere BOOTSTRAP_SETUP_TOKEN + email bootstrap explícito.
    # - Si la base está vacía, el bootstrap es el camino recomendado para iniciar.
    bootstrap_needed = _bootstrap_needed()
    if bootstrap_needed:
        st.warning("⚙️ Bootstrap requerido (no hay usuarios en la base). Usa 'Ingreso Administrador Principal' para crear admins iniciales.")

    colA, colB = st.columns([1, 1])

    with colA:
        st.subheader("👤 Ingreso con Contraseña")
        st.caption("Opción principal. Si es tu primera vez, después de validar tu email podrás definir contraseña.")
        ...
        if submit:
            if bootstrap_needed:
                st.error("Aún no hay usuarios cargados. Usa 'Ingreso Administrador Principal' primero.")
            else:
    ...
    with colB:
        st.subheader("🔐 Otras formas de ingreso")

        with st.expander("🎟️ Ingreso con Código de un Uso (usuario nuevo)", expanded=False):
            ...
            if submit2:
                if bootstrap_needed:
                    st.error("Aún no hay usuarios cargados. Primero ejecuta el bootstrap (Admin Principal).")
                else:
        ...
        with st.expander("🛡️ Ingreso Administrador Principal (Bootstrap)", expanded=False):
            st.caption(
                "Usa esta opción para crear los admins iniciales si la base está vacía, "
                "o para recuperar acceso de un admin bootstrap (redefinir contraseña)."
            )
            with st.form("bootstrap_always_form"):
                b_email = st.text_input("Email bootstrap", placeholder="bnbpartnerscommunity@gmail.com")
                b_token = st.text_input("BOOTSTRAP_SETUP_TOKEN", type="password")
                b_submit = st.form_submit_button("Continuar")

            if b_submit:
                try:
                    if not BOOTSTRAP_SETUP_TOKEN:
                        raise RuntimeError("Falta BOOTSTRAP_SETUP_TOKEN en secrets.")
                    if b_token.strip() != BOOTSTRAP_SETUP_TOKEN.strip():
                        raise RuntimeError("BOOTSTRAP_SETUP_TOKEN inválido.")

                    em = _norm_email(b_email)
                    if not em or em not in BOOTSTRAP_ADMINS:
                        raise RuntimeError("Email no autorizado como bootstrap admin.")

                    # Si aún no existen usuarios, creamos el set completo de bootstrap admins.
                    if bootstrap_needed:
                        _bootstrap_create_admins(b_token)

                    # Si ya existe la base, verificamos que el email exista; si no, lo creamos.
                    dfu = get_users_df()
                    exists = (not dfu.empty) and (dfu["email"].astype(str).str.lower() == em).any()
                    if not exists:
                        now = _now_iso()
                        row = {
                            "user_id": str(uuid.uuid4()),
                            ...
                            "communities_csv": "*",
                            "pw_salt": _rand_salt(),
                            "pw_hash": "",  # se setea al entrar
                            "created_at": now,
                            "last_login": "",
                        }
                        _append_row(TAB_USERS, USERS_HEADERS, row)

                    st.session_state["pw_setup_email"] = em
                    st.success("Listo. Ahora define/redefine la contraseña para este admin.")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
