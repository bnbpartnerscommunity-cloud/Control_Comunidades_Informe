...
def get_communities_df() -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        # Evita duplicados (p. ej. por testing o escrituras repetidas):
        # nos quedamos con el último registro por community_id.
...
def module_communities(user: dict):
    ...
    with st.expander("🛠️ Modificar comunidad (estado, foto, instalaciones y tareas)", expanded=True):
        ...
        st.markdown("### ➕ Añadir instalación")
        with st.form(key=f"add_installation_form_{comm_id}", clear_on_submit=True):
