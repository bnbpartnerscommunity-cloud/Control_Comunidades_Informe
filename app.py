...
def _hash_with_pepper(value: str, salt: str) -> str:
    ...
    Nota: mantenemos el nombre por compatibilidad con el resto del código.
    """
    # Normaliza entradas
    salt_b = (salt or "").encode("utf-8")
    pwd_b = (str(value) + str(APP_PEPPER)).encode("utf-8")

    dk = hashlib.pbkdf2_hmac("sha256", pwd_b, salt_b, 200_000, dklen=32)
...
def _set_user_password(email: str, new_password: str):
    ...
    salt = _rand_salt()  # siempre rotamos salt al cambiar contraseña
...
def _verify_password(email: str, password: str) -> Optional[pd.Series]:
    ...
    return r if _secure_eq(got, expected) else None
...
def login_screen():
    ...
    st.caption("Acceso seguro por invitación (Admin) + contraseña. También incluye acceso de recuperación (Admin Principal).")
    ...
    # ============================================================
    st.markdown("### ✅ Ingreso con Contraseña")
    ...
    if submit:
        ...
        if not user:
            ...
            # Si no tiene password aún, forzamos setear (primer ingreso / invitación recién activada)
        ...
        if ok:
            if len(p1) < 8:
                ...
                try:
                    ...
                    # Auto-login para validar inmediatamente
                    user_after = _verify_password(st.session_state["pw_setup_email"], p1)
                    if user_after is None:
                        raise RuntimeError("Contraseña guardada pero no pudo verificarse. Revisa APP_PEPPER / duplicados en Users.")
                    _login_user(user_after.to_dict())
                    st.success("Contraseña guardada. Iniciaste sesión automáticamente.")
    ...
    # ============================================================
    # 3) Admin Principal (Bootstrap / Recovery) (SIEMPRE DISPONIBLE, plegado)
    # ============================================================
    with st.expander("🧰 Ingreso Administrador Principal (Bootstrap / Recuperación)", expanded=False):
        st.warning(
            "Modo de recuperación. Requiere BOOTSTRAP_SETUP_TOKEN + email autorizado en BOOTSTRAP_ADMIN_EMAILS. "
            "Úsalo solo si un Admin Principal pierde acceso o para el primer ingreso si aún no hay contraseña."
        )

        if not BOOTSTRAP_SETUP_TOKEN:
            st.error("Falta BOOTSTRAP_SETUP_TOKEN en secrets. No se puede usar este modo.")
        elif not BOOTSTRAP_ADMINS:
            st.error("Falta BOOTSTRAP_ADMIN_EMAILS en secrets.")
        else:
            no_users = _bootstrap_needed()

            if no_users:
                st.info("No hay usuarios en la base. Primero debes crear los Admins iniciales (bootstrap).")
                with st.form("bootstrap_create_admins_form"):
                    token = st.text_input("BOOTSTRAP_SETUP_TOKEN", type="password", key="bs_token_create")
                    create_btn = st.form_submit_button("Crear Admins iniciales")
                if create_btn:
                    try:
                        _bootstrap_create_admins(token)
                        st.success("Admins bootstrap creados. Ahora puedes definir la contraseña del Admin Principal aquí mismo.")
                        st.rerun()
                    except Exception as e:
                        st.error(str(e))

            st.markdown("#### Definir / Resetear contraseña del Admin Principal")
            with st.form("bootstrap_reset_form"):
                token2 = st.text_input("BOOTSTRAP_SETUP_TOKEN", type="password", key="bs_token_reset")
                admin_email = st.text_input("Email Admin Principal (bootstrap)", placeholder="bnbpartnerscommunity@gmail.com").strip().lower()
                newp1 = st.text_input("Nueva contraseña", type="password", key="bs_newp1")
                newp2 = st.text_input("Repite contraseña", type="password", key="bs_newp2")
                do_reset = st.form_submit_button("Guardar y entrar")

            if do_reset:
                try:
                    if token2.strip() != BOOTSTRAP_SETUP_TOKEN.strip():
                        raise RuntimeError("Token inválido.")
                    if _norm_email(admin_email) not in BOOTSTRAP_ADMINS:
                        raise RuntimeError("Email no autorizado como Admin Principal.")
                    if len(newp1) < 10:
                        raise RuntimeError("Usa una contraseña de al menos 10 caracteres.")
                    if newp1 != newp2:
                        raise RuntimeError("Las contraseñas no coinciden.")

                    # Si el usuario no existe (por ejemplo, lo borraron), lo recreamos como admin bootstrap.
                    users_df = get_users_df()
                    em = _norm_email(admin_email)
                    if users_df.empty or not (users_df["email"].astype(str).str.lower() == em).any():
                        now = _now_iso()
                        row = {
                            "user_id": str(uuid.uuid4()),
                            "email": em,
                            ...
                            "communities_csv": "*",
                            "pw_salt": _rand_salt(),
                            "pw_hash": "",  # se setea abajo
                            "created_at": now,
                            "last_login": "",
                        }
                        _append_row(TAB_USERS, USERS_HEADERS, row)

                    _set_user_password(em, newp1)

                    # Auto-login inmediato
                    user_after = _verify_password(em, newp1)
                    if user_after is None:
                        raise RuntimeError("Contraseña guardada pero no pudo verificarse. Revisa APP_PEPPER / duplicados en Users.")
                    _login_user(user_after.to_dict())
                    st.success("Acceso Admin Principal listo. Iniciaste sesión.")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
