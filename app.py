...
APP_PEPPER = get_secret("APP_PEPPER", "CHANGE_ME")
BOOTSTRAP_ADMIN_EMAILS = [
    x.strip().lower()
    for x in get_secret("BOOTSTRAP_ADMIN_EMAILS", "").split(",")
    if x.strip()
]
INVITE_EXPIRY_HOURS = int(get_secret("INVITE_EXPIRY_HOURS", "48"))
...
# ---------------------------
# Hashing: password + invite codes (stdlib)
# ---------------------------
...
def hash_invite_code(code: str, pepper: str) -> str:
    # SHA256(code + pepper) – suficiente para código temporal (no guardamos el código en claro)
...
def ensure_sheet_tabs_and_headers():
    ...
    def ensure_headers(tab: str, headers: List[str]):
        ...
        # If different, overwrite row1 (simple and safe for MVP)
...
def read_table(tab: str) -> Tuple[List[str], List[Dict[str, str]]]:
    ...
    for r in values[1:]:
        ...
        # ignore empty row (no key columns)
        rows.append(d)
    return headers, rows
...
def bootstrap_admin_users():
    """
    Ensures BOOTSTRAP_ADMIN_EMAILS exist as active admins (no password yet).
    """
...
def generate_invite(email: str, created_by: str) -> str:
    code = "".join(str(secrets.randbelow(10)) for _ in range(6))  # 6-digit
...
def find_valid_invite(email: str, code: str) -> Optional[Dict[str, str]]:
    ...
    for inv in reversed(invites):  # newest first
...
def mark_invite_used(invite: Dict[str, str]):
    headers, invites = read_table("Invites")
    # find exact row by (email + created_at) match (good enough for MVP)
...
def create_community(name: str) -> Dict[str, str]:
    ...
    # generate new COM id incremental
...
st.title("🛡️ Control Edificio Pro — Acceso y Administración (MVP)")
...
if not auth:
    ...
    with c2:
        ...
        st.caption("Si te crearon usuario, usa el código temporal para definir tu contraseña.")
...
# Admins: optionally see all communities even without access rows
if auth.is_admin:
...
if not allowed:
    ...
    st.info("Siguiente paso: aquí conectaremos el módulo de Reportes (Draft/Final) y el checklist por reporte.")
...
if auth.is_admin:
    st.divider()
    st.header("🧑‍💼 Panel Admin (Héctor y admins preliminares)")
    ...
    with tab_u:
        st.subheader("Crear usuario y generar código (48h)")
        ...
        is_admin = st.checkbox("¿Es admin global?", value=False)
            ...
            if not EMAIL_RE.match(em):
                ...
                if not existing:
                    u = {
                        ...
                        "is_active": "TRUE",
                        "password_hash": "",  # se setea tras validar código
                        "created_at": iso(now_utc()),
                        "last_login_at": "",
                    }
                else:
                ...
                # grant access for selected comms
...
st.caption("MVP listo: autenticación + admin + comunidades (persistente en Sheets). Próximo: Reports (draft/final) + Checklist por reporte + Drive fotos.")
