            ...
            if ok:
                if len(p1) < 8:
                    ...
                    try:
                        ...
                        # Auto-login para evitar confusiones y validar inmediatamente el hash guardado
                        user_after = _verify_password(st.session_state["pw_setup_email"], p1)
                        if user_after is None:
                            raise RuntimeError("Contraseña guardada pero no pudo verificarse. Revisa APP_PEPPER / datos en Users.")
