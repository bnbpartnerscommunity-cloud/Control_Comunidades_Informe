...
def _dedupe_users_df(df: pd.DataFrame) -> pd.DataFrame:
    """Elimina duplicados por email (deja el registro más reciente).
    Esto evita que el login tome un registro viejo sin password o con hash distinto.
    """
    if df is None or df.empty:
        return df

    d = df.copy()
    d["__email_norm"] = d["email"].astype(str).str.strip().str.lower()

    # Orden: preferir el que tenga last_login más reciente; si no, created_at.
    def _to_dt(s):
        return pd.to_datetime(s, errors="coerce")

    d["__last"] = d["last_login"].apply(_to_dt) if "last_login" in d.columns else pd.NaT
    d["__created"] = d["created_at"].apply(_to_dt) if "created_at" in d.columns else pd.NaT

    d = d.sort_values(by=["__email_norm", "__last", "__created"], ascending=[True, True, True])

    # keep last row per email
    dedup = d.drop_duplicates(subset="__email_norm", keep="last").copy()
    dedup = dedup.drop(columns=[c for c in ["__email_norm", "__last", "__created"] if c in dedup.columns])

    # Mantener exactamente las columnas esperadas (en el mismo orden) si existen
    cols = [c for c in USERS_HEADERS if c in dedup.columns]
    return dedup[cols]
...
def _set_user_password(email: str, new_password: str):
    df = get_users_df()
    df = _dedupe_users_df(df)
    em = _norm_email(email)

    row = df[df["email"].astype(str).str.strip().str.lower() == em]
    if row.empty:
        raise RuntimeError("Usuario no existe.")

    # Tomamos el registro deduplicado (único) y actualizamos por user_id
    r = row.iloc[0]
    user_id = str(r.get("user_id", "")).strip()
    salt = str(r.get("pw_salt", "")).strip() or _rand_salt()
    pw_hash = _hash_with_pepper(new_password, salt)

    if not user_id:
        # fallback: actualizar por email
        mask = df["email"].astype(str).str.strip().str.lower() == em
    else:
        mask = df["user_id"].astype(str).str.strip() == user_id

    df.loc[mask, "pw_salt"] = salt
    df.loc[mask, "pw_hash"] = pw_hash

    # sanity check inmediato (mismo proceso)
    chk_salt = str(df.loc[mask, "pw_salt"].iloc[0])
    chk_hash = str(df.loc[mask, "pw_hash"].iloc[0])
    if _hash_with_pepper(new_password, chk_salt) != chk_hash:
        raise RuntimeError("Contraseña guardada pero no pudo verificarse. Revisa APP_PEPPER.")
...
def _verify_password(email: str, password: str) -> Optional[pd.Series]:
    ...
    rows = df[df["email"].astype(str).str.strip().str.lower() == em]
    if rows.empty:
        return None

    # Solo activos
    rows = rows[rows["status"].astype(str).str.strip().str.lower() == "active"]
    if rows.empty:
        return None

    # Preferir el que tenga password ya seteada
    with_pw = rows[rows["pw_hash"].astype(str).str.strip() != ""]
    if not with_pw.empty:
        r = with_pw.iloc[0]
    ...
    # Si no hay password aún: permite primer ingreso, se pedirá setear
    return rows.iloc[0]
