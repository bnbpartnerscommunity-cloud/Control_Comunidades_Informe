...
DRAFT_EXPIRY_HOURS = int(_sec("DRAFT_EXPIRY_HOURS", "168"))  # 7 días por defecto
...
AUDIT_HEADERS = [
    "audit_id", "ts", "user_email", "action", "entity_type", "entity_id", "details"
]
...
def _service_account_info() -> dict:
    ...
    Importante:
    - Para Service Accounts, `token_uri` es obligatorio. En Streamlit Secrets a veces no está,
      así que ponemos defaults seguros.
    - Normalizamos la private key (
 -> salto real) porque en secrets suele venir escapada.
    """
...
")
    info = {
        ...
        # Defaults oficiales
        "auth_uri": (GCP_AUTH_URI or "https://accounts.google.com/o/oauth2/auth"),
        "token_uri": (GCP_TOKEN_URI or "https://oauth2.googleapis.com/token"),
        "auth_provider_x509_cert_url": (GCP_AUTH_PROVIDER_X509_CERT_URL or "https://www.googleapis.com/oauth2/v1/certs"),
        "client_x509_cert_url": GCP_CLIENT_X509_CERT_URL,
    }
    # Elimina claves None/"" para evitar comportamientos raros
...
def _invalidate_cache():
    """Limpia caches de lectura después de escrituras en Sheets."""
    try:
        st.cache_data.clear()
    except Exception:
        pass
...
def compress_image_to_b64(
    ...
    max_side: int = 1280,
    quality: int = 70,
    max_b64_chars: int = 45000,
) -> tuple[Optional[str], Optional[str], Optional[str]]:
    """Comprime/redimensiona una imagen y la devuelve como base64 JPEG.
    Retorna (b64, mime, error_msg)."""
    try:
        ...
        if len(b64) > max_b64_chars:
            return None, None, "Imagen demasiado pesada incluso comprimida. Usa una foto más liviana."
        return b64, "image/jpeg", None
    except Exception as e:
        return None, None, f"No se pudo procesar la imagen: {e}"
...
# ============================================================
# DRIVE (Service Account) — Fase 1
#   - No OAuth por usuario final (evita verificación/testers)
#   - La app controla RBAC; Drive actúa como storage "corporativo"
# ============================================================
...
def drive_download_bytes(file_id: str) -> bytes:
    """Descarga bytes desde Drive usando service account.
    Útil para ofrecer 'Descargar' sin exponer links públicos.
    """
    svc = drive_service()
    ...
    while not done:
        _, done = _sheets_call(downloader.next_chunk)
    return fh.getvalue()
...
def _get_users_df_cached() -> pd.DataFrame:
    return _sheet_to_df(TAB_USERS, USERS_HEADERS)

def get_users_df() -> pd.DataFrame:
    return _get_users_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_invites_df_cached() -> pd.DataFrame:
    return _sheet_to_df(TAB_INVITES, INVITES_HEADERS)

def get_invites_df() -> pd.DataFrame:
    return _get_invites_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_communities_df_cached() -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        if "community_id" in df.columns:
            df = df.drop_duplicates(subset=["community_id"], keep="last").copy()
    return df

def get_communities_df() -> pd.DataFrame:
    return _get_communities_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_installations_df_cached() -> pd.DataFrame:
    df = _sheet_to_df(TAB_INSTALLATIONS, INSTALLATIONS_HEADERS)
    if not df.empty and "installation_id" in df.columns:
        df = df.drop_duplicates(subset=["installation_id"], keep="last").copy()
    return df

def get_installations_df() -> pd.DataFrame:
    return _get_installations_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_tasks_df_cached() -> pd.DataFrame:
    df = _sheet_to_df(TAB_TASKS, TASKS_HEADERS)
    if not df.empty and "task_id" in df.columns:
        df = df.drop_duplicates(subset=["task_id"], keep="last").copy()
    return df

def get_tasks_df() -> pd.DataFrame:
    return _get_tasks_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_reports_df_cached() -> pd.DataFrame:
    df = _sheet_to_df(TAB_REPORTS, REPORTS_HEADERS)
    if not df.empty and "report_id" in df.columns:
        df = df.drop_duplicates(subset=["report_id"], keep="last").copy()
    return df

def get_reports_df() -> pd.DataFrame:
    return _get_reports_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_reportitems_df_cached() -> pd.DataFrame:
    df = _sheet_to_df(TAB_REPORTITEMS, REPORTITEMS_HEADERS)
    if not df.empty and "report_item_id" in df.columns:
        df = df.drop_duplicates(subset=["report_item_id"], keep="last").copy()
    return df

def get_reportitems_df() -> pd.DataFrame:
    return _get_reportitems_df_cached().copy()

@st.cache_data(ttl=30, show_spinner=False)
def _get_audit_df_cached() -> pd.DataFrame:
    df = _sheet_to_df(TAB_AUDIT, AUDIT_HEADERS)
    if not df.empty and "audit_id" in df.columns:
        df = df.drop_duplicates(subset=["audit_id"], keep="last").copy()
    return df

def get_audit_df() -> pd.DataFrame:
    return _get_audit_df_cached().copy()


def log_audit(user_email: str, action: str, entity_type: str = "", entity_id: str = "", details: str = ""):
    try:
        row = {
        ...
        _append_row(TAB_AUDIT, AUDIT_HEADERS, row)
    except Exception:
        # Nunca debe botar la app por fallos de auditoría
        pass
...
def _login_user(user_row: dict):
    ...
    log_audit(st.session_state["auth_user"]["email"], "login", "user", user_row.get("user_id",""), "Ingreso exitoso")
...
def create_invite(email: str, role: str, role_label: str, communities_csv: str, created_by: str) -> str:
    ...
    log_audit(created_by, "invite_create", "invite", invite_id, f"role={role} communities={communities_csv}")
    return code_plain
...
def create_report(community_id: str, report_date: str, created_by: str) -> str:
    ...
    log_audit(created_by, "report_create", "report", rid, f"community_id={community_id} date={report_date}")
    return rid
...
def get_latest_draft_report(community_id: str) -> Optional[str]:
    ...
    latest = df2.iloc[0]
    # expiración
    if parse_dt(latest["updated_at"]) < NOW() - timedelta(hours=DRAFT_EXPIRY_HOURS):
        return None
...
def drive_oauth_ui(user: dict):
    """UI de estado para Drive en Fase 1 (Service Account).
    No hay OAuth por usuario final. Si falla, es por permisos de carpeta o secrets.
    """
    st.sidebar.markdown("### 🗂️ Drive (Service Account)")
    if not DRIVE_REPORTS_FOLDER_ID:
        st.sidebar.error("Falta DRIVE_REPORTS_FOLDER_ID en secrets (ID de carpeta destino).")
        return

    # Smoke test: intentar leer metadata mínima de la carpeta
    try:
        svc = drive_service()
        meta = _sheets_call(svc.files().get(fileId=DRIVE_REPORTS_FOLDER_ID, fields="id,name").execute)
        st.sidebar.success(f"Drive listo ✅ Carpeta: {meta.get('name','(sin nombre)')}")
    except Exception as e:
        st.sidebar.error("Drive no accesible. Revisa que la carpeta esté compartida con el service account.")
...
def module_communities(user: dict):
    ...
    with st.expander("➕ Crear / 🗑️ Eliminar comunidades", expanded=True):
        ...
        if comms.empty:
            ...
            for i, (_, r) in enumerate(comms.sort_values(["name"]).iterrows()):
                ...
                with right:
                    ...
                    confirm_key = f"del_confirm_{cid}_{i}"
                    confirm = st.text_input("Confirma", placeholder="ELIMINAR", key=confirm_key, label_visibility="collapsed")
                    if st.button("🗑️ Eliminar", key=f"del_btn_{cid}_{i}", disabled=(confirm.strip().upper() != "ELIMINAR")):
                        try:
    ...
    with st.expander("🛠️ Modificar comunidad (estado, foto, instalaciones y tareas)", expanded=True):
        ...
        with st.form(key=f"add_inst_form_{comm_id}", clear_on_submit=True):
            ...
            submitted = st.form_submit_button("Guardar instalación")
            if submitted:
                try:
...
def module_informes(user: dict):
            ...
            for _, row in minst.iterrows():
                ...
                with box:
                    ...
                    with cR:
                        ...
                        photo_b64_new = None
                        if up is not None:
                            photo_b64_new, photo_mime_new, err = compress_image_to_b64(up)
                            if err:
                                st.error(err)
                                photo_bytes = None
                                photo_mime = ""
                            else:
                                photo_bytes = base64.b64decode(photo_b64_new)
                                photo_mime = photo_mime_new or "image/jpeg"
                                st.image(photo_bytes, use_container_width=True, caption="Vista previa (comprimida)")
                ...
                out = {
                    ...
                    "photo_b64": (photo_b64_new if photo_b64_new else (cur_photo_b64 or "")),
                    "updated_at": _now_iso(),
                }
    ...
    if st.button("💾 Guardar cambios", use_container_width=True):
        try:
            ...
            log_audit(user["email"], "report_save", "report", report_id, f"status={new_status}")
            st.success("Guardado ✅")
            st.rerun()
        except Exception as e:
            st.error(str(e))
